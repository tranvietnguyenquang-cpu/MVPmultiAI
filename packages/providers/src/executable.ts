...
export async function resolveCliExecutable(
  ...
  const candidates = pathValue
    ...
    .filter(Boolean)
    .flatMap((folder) => names.map((name) => path.join(folder, name)));

  for (const candidate of candidates) {
    try {
      await accessFile(candidate);
      return { command, canonicalPath: await canonicalize(candidate) };
    } catch {
      // Continue through the fixed allowlisted executable candidates.
    }
  }
  return undefined;
}
