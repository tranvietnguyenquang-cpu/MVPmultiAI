...
describe("provider health probes",()=>{
  ...
  it("rejects a Claude session that requests workspace-write instead of silently downgrading it",async()=>{const provider=new ClaudeCliProvider(mockSpawn([]),resolver);await expect(provider.createSession({workspace:"C:\\MVPmultiAI",taskId:"task",capability:"WORKSPACE_WRITE"})).rejects.toThrow(/does not support/i);});
