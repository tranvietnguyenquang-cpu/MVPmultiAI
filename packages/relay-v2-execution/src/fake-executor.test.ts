...
describe("FakeExecutor contract", () => {
  it("emits deterministic in-process output and warnings with a fast injected clock", async () => {
    ...
    const prepared = await executor.prepare({ sessionId: "session", taskId: "task", projectId: "project", workspacePath: "C:\\fake", approvedExecutor: "AUTO", approvedModel: "AUTO", approvedEffort: "MEDIUM", title: "Fake", objective: "Fake" });
